...
export default class ExportConfig extends Command {
  static description = 'Import Keycloak configuration from /keycloak/conf/settings.json'

  static examples = [
    `<%= config.bin %> <%= command.id %>
keycloak importConfig
`,
  ]
  ...
  async run(): Promise<void> {
    ...
    //   },
    // )
    exec("docker-compose exec -t addictive-keycloak /opt/keycloak/bin/kc.sh import --file /opt/keycloak/conf/settings.json", (error: any, stdout: any, stderr: any) => {
      if (error) {
